...
import { useAddItemSerial } from "../react-query/itemsserial/useCreateItemSerial";
...
const TranPOForm = () => {
  ...
  const update_Batch = (data) => {
    ...
    // update header
    updateTran({ id, ...fields, t_no, t_date });
    //add details
    batchdetls.forEach((rec) => {
      const { id, tl_id, ...fields } = rec;
  ...
  const onSubmit = (values) => {
    ...
    if (status === "add") {
      ...
      addAuditlog(auditdata);
      // const { id, doc_id, doc_purchase, doc_abbre, ...fields } =
      //   documentno[0] || 0;
      ...
      // const newdata = { ...values, t_no: newdocno };
      // const updDocno = { ...fields, doc_purchase: newno, doc_abbre };
      //updateDocumentno({ id, ...updDocno });
