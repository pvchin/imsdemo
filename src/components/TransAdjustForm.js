...
const TransAdjustForm = () => {
  ...
  const update_Batch = (data) => {
    ...
    const { id, ta_id, ta_batchno, ...fields } = data;
    // delete old details
    //deleteTransadjustDetls({ tad_batchno: ta_batchno });
    tranadjustdetls
      .filter((r) => r.tad_batchno === ta_batchno)
      .forEach((rec) => {
        deleteTransadjustDetls(rec.id);
      });

    // update header
    updateTransadjust({ id, ...fields, ta_batchno });
    //add details
    batchdetls.forEach((rec) => {
      console.log("upd batch", rec);
      const { id, tad_id, ...fields } = rec;
      addTransadjustDetls({ ...fields, tad_batchno: ta_batchno });
    });
  };
